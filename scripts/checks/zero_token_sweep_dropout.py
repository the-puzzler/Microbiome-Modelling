#!/usr/bin/env python3
"""
Sweep the number of zero-valued "text" tokens and see how much they help
gingivitis dropout performance for:
  - OTU-only checkpoint
  - Text-trained checkpoint

We take the standard gingivitis dropout setup and, for each checkpoint and
each n_dummy in a sweep, do:
  - Build OTU embeddings for each SRS.
  - Project OTUs with input_projection_type1.
  - Concatenate n_dummy extra positions with zero hidden state.
  - Run the transformer and output head.
  - Evaluate dropout AUC/AP using only the OTU positions.
"""
...
def score_with_n_zero_tokens(
    ...
    """
    Score OTUs for a single SRS, concatenating n_zero_tokens dummy positions
    with zero hidden state after the OTUs.
    """
    ...
    with torch.no_grad():
        ...
        if n_zero_tokens > 0:
            z2 = torch.zeros((1, n_zero_tokens, shared_utils.D_MODEL), dtype=torch.float32, device=device)
            h = torch.cat([h1, z2], dim=1)
            mask = torch.ones((1, n1 + n_zero_tokens), dtype=torch.bool, device=device)
        else:
            h = h1
            mask = torch.ones((1, n1), dtype=torch.bool, device=device)

        h = model.transformer(h, src_key_padding_mask=~mask)
        logits = model.output_projection(h).squeeze(-1)  # (1, n1 + n_zero_tokens or n1)
...
def eval_checkpoint_for_sweep(
    ...
    with h5py.File(shared_utils.PROKBERT_PATH) as emb_file:
        ...
        for n_zero in n_zero_list:
            print(f"[{label}] scoring with {n_zero} zero tokens per SRS...")
            subject_time_scores = {}
            ...
            for (subject, time_code), srs_list in tqdm(
                ...
                sample_dicts = []
                for srs in srs_list:
                    sdict = score_with_n_zero_tokens(
                        srs,
...
def main():
    ...
    # Sweep over different numbers of zero tokens per SRS
