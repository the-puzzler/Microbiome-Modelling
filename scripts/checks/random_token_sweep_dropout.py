#!/usr/bin/env python3
"""
Sweep the number of random-valued "text" tokens and see how much they affect
gingivitis dropout performance for:
  - OTU-only checkpoint
  - Text-trained checkpoint

This is analogous to zero_token_sweep_dropout.py, but instead of appending
zero hidden vectors, we append random hidden vectors (per evaluation run,
fixed within a forward pass) to test whether arbitrary extra content helps
or hurts compared to zero tokens.
"""
...
def score_with_n_random_tokens(
    srs,
    micro_to_otus,
    resolver,
    model,
    ...
    """
    Score OTUs for a single SRS, concatenating n_rand_tokens dummy positions
    with random hidden state after the OTUs.
    """
    ...
    with torch.no_grad():
        h1 = model.input_projection_type1(x1)  # (1, n1, d_model)
        if n_rand_tokens > 0:
            # Random hidden vectors ~ N(0, 1) per token; scale matches typical activations
            z2 = torch.randn((1, n_rand_tokens, shared_utils.D_MODEL), dtype=torch.float32, device=device)
            h = torch.cat([h1, z2], dim=1)
            mask = torch.ones((1, n1 + n_rand_tokens), dtype=torch.bool, device=device)
        else:
            h = h1
            mask = torch.ones((1, n1), dtype=torch.bool, device=device)

        h = model.transformer(h, src_key_padding_mask=~mask)
        logits = model.output_projection(h).squeeze(-1)  # (1, n1 + n_rand_tokens or n1)
...
def eval_checkpoint_for_sweep(
    ...
    with h5py.File(shared_utils.PROKBERT_PATH) as emb_file:
        ...
        for n_rand in n_rand_list:
            print(f"[{label}] scoring with {n_rand} random tokens per SRS...")
            subject_time_scores = {}
            subject_time_presence = {}

            for (subject, time_code), srs_list in tqdm(
                list(subject_time_to_srs.items()),
                desc=f"[{label}] groups (n_rand={n_rand})",
            ):
                sample_dicts = []
                for srs in srs_list:
                    sdict = score_with_n_random_tokens(
                        srs,
            ...
            if y_true.size and np.unique(y_true).size > 1:
                ...
                auc = float("nan")
                ap = float("nan")
            print(f"[{label}] n_rand={n_rand} — AUC: {auc:.4f} | AP: {ap:.4f}")
            results[n_rand] = (auc, ap)
...
def main():
    ...
    # Sweep over different numbers of random tokens per SRS
    n_rand_list = [0, 2, 4, 8, 16, 32]
    ...
    print("Checkpoint\t#random_tokens\tAUC")
    for ckpt_label, res in [
        ("no-text", res_no_text_ckpt),
        ("text-trained", res_text_ckpt),
    ]:
        for n_rand in n_rand_list:
            auc = res.get(n_rand, (float("nan"),))[0]
            print(f"{ckpt_label}\t{n_rand}\t{auc:.4f}")
