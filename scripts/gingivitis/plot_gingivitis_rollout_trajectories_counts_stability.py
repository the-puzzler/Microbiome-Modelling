...
OUT_PNG = os.path.join("data", "gingivitis", "gingivitis_rollout_trajectories_counts_stability.png")
CACHE_NPZ = os.path.join("data", "gingivitis", "gingivitis_rollout_trajectories_counts_stability_cache.npz")


def parse_args():
    p = argparse.ArgumentParser(description="Gingivitis: plot rollout trajectories (counts + stability vs step).")
...
def list_rollout_keys(rollout_tsv):
    ...
    return sorted(keys, key=lambda x: (x[0], time_key(x[1])))
...
def main():
    ...
    with h5py.File(shared_utils.PROKBERT_PATH) as emb_file:
        ...
        for key in picked_keys:
            ...
            for i, r in enumerate(tqdm(rows_sub, desc=f"Stability {key[0]} {key[1]}", unit="step", leave=False)):
    ...
    fig.suptitle("Gingivitis rollout trajectories — OTU count and stability per step", y=0.995)
    plt.tight_layout()
