...
def eval_model(name, base_estimator, X_ml, Y_ml, M_ml, groups_ml, SEED):
    ...
    for fold, (tr_idx, te_idx) in enumerate(gkf.split(X_ml, groups=groups_ml), 1):
        ...
        # ---- Label diagnostics on TRAIN split ----
        total_labels = Y_ml.shape[1]
        no_train_examples = 0
        single_class_train = 0
        for j in range(total_labels):
            elig = M_ml[tr_mask, j]  # eligible = absent at t1
            if not np.any(elig):
                no_train_examples += 1
                continue
            y_tr = Y_ml[tr_mask, j][elig]
        ...
        clf.fit(X_ml[tr_mask], Y_ml[tr_mask])
        # predict_proba is available for both LR and RF under OvR
        prob = clf.predict_proba(X_ml[te_mask])

        # ---- Masked per-label AUC on TEST split ----
        per_label = []
        for j in range(total_labels):
            mask = M_ml[te_mask, j]
            if not np.any(mask):
...
# ---- Run both ----
eval_model("LogReg (OvR)", lr_est, X_ml, Y_ml, M_ml, groups_ml, SEED)
eval_model("RandForest (OvR)", rf_est, X_ml, Y_ml, M_ml, groups_ml, SEED)
