#!/usr/bin/env python3
"""
DIABIMMUNE: Multilabel Colonisation and Dropout (t1 -> t2) baselines
using a small MLP classifier instead of Logistic Regression / Random Forest.

The data preprocessing, subject subsampling, and masking logic follow the
existing `baselines_colonisation.py` and `baselines_binary_taxa.py` scripts.
"""
...
def _load_diabimmune_presence():
    """
    Build binary taxa presence per (subject, age) as in the DIABIMMUNE baselines,
    including optional subject subsampling.
    """
    ...
    print("Total DIABIMMUNE subjects with age-resolved samples: {}".format(len(all_subjects)))
    if N_SUBJECTS_SUBSAMPLE is not None and len(all_subjects) > N_SUBJECTS_SUBSAMPLE:
        ...
        subject_time_to_srs = defaultdict(
            list,
            {k: v for k, v in subject_time_to_srs.items() if k[0] in keep},
        )
        print(
            "Subsampling DIABIMMUNE subjects to n={}; subject-time groups retained: {}".format(
                N_SUBJECTS_SUBSAMPLE, len(subject_time_to_srs)
            )
        )
    else:
        print(
            "Using all DIABIMMUNE subjects; subject-time groups: {}".format(
                len(subject_time_to_srs)
            )
        )
...
def _build_multilabel_task(mode):
    ...
    matching DIABIMMUNE baseline scripts.
    """
    X, kept_otus, keys, st_presence, key_to_row = _load_diabimmune_presence()
...
def run_diabimmune_mlp():
    # Colonisation
    print("=== DIABIMMUNE: Multilabel Colonisation (MLP) ===")
    ...
    # Dropout
    print("\n=== DIABIMMUNE: Multilabel Dropout (MLP) ===")
