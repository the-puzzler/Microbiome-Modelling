...
OUT_TSV = os.path.join("data", "diabimmune", "visionary_rollout_prob.tsv")
SAMPLES_CSV = os.path.join("data", "diabimmune", "samples.csv")


def parse_args():
    p = argparse.ArgumentParser(description="Run rollouts from DIABIMMUNE (subject, age_at_collection_in_days) starts.")
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--n-starts", type=int, default=1000, help="Randomly sample this many real start points.")
...
def main():
    ...
    if args.n_starts and len(starts_all) > args.n_starts:
